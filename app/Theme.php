...
class Theme extends Model
{
    ...
    public function testConstructorRegex(){
        $themes = Theme::get();
        foreach($themes as $t){
            $regex = Tools::regexConstructor($t->subcategory);
        }
    }
                    ...
                    if(is_array($category)){
                        foreach($category as $subcategory){
                            ...
                            $checkInDB = DB::table('themes')
                                            ->where('theme', RF::if($themeRegex[0]))
                                            ->where('category', RF::if($categoryRegex[0]))
                                            ->where('subcategory', RF::if($subcategoryRegex[0]))
                                            ->first();
                            if(!$checkInDB){

                                try {
                                    $themeEntity = new Theme();
                                    $themeEntity->theme = RF::if($themeRegex[0]);
                                    $themeEntity->theme_regex = RF::if($themeRegex[1]);
                                    $themeEntity->category = RF::if($categoryRegex[0]);
                                    $themeEntity->category_regex = RF::if($categoryRegex[1]);
                                    $themeEntity->subcategory = RF::if($subcategoryRegex[0]);
                                    $themeEntity->subcategory_regex = RF::if($subcategoryRegex[1]);
                                    $themeEntity->save();
                                } catch (\Throwable $th) {
                                    dd($th);
                                }
    ...
    public function getCategoriesRegex(){
        return 
        [
            'test@[Tt]est'=>['test@[Tt]est'=>['test@[Tt]est']],
            'famille@[fF]amilles?'=>[
                'identité@[Ii]dentit[ée]s?'=>[
                    "carte d\'identité@[Cc]artes?[\w\'\s]{0,10}identit[ée]s?", 
                    'actes et registres@[Aa]ctes?[\s]{0,10}registres?', 
                    'nationalité@[Nn]ationalit[ée]s?', 
                    'changement de nom@[Cc]hangements?[\w\'\s]{0,10}noms?', 
                    'transgenre@[Tt]ransgenres?'
                ], 
                'couple@[Cc]ouples?'=>[
